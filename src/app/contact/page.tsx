...
import {
  Mail, Phone, MapPin, Clock, Instagram, Linkedin, Newspaper,
  Handshake, Loader2, Check, TrendingUp, Users, Award, Building2
} from "lucide-react";
...
.glass-card { background:rgba(255,255,255,0.05); backdrop-filter:blur(16px); -webkit-backdrop-filter:blur(16px); border:1px solid rgba(255,255,255,0.15); box-shadow:0 4px 30px rgba(0,0,0,0.1); }
...
const BusinessMetricsCard = () => (
  <div className="glass-card rounded-2xl p-8 shadow-lg">
    ...
    <div className="grid grid-cols-3 gap-6 text-center">
      <div>
        <div className="metric-highlight text-3xl font-bold">4.9</div>
  ...
  <>
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-4">
        <MapPin className="h-6 w-6" style={{ color: "#fcd129" }} />
        <p>Oneplace 8th floor 807, Salunke Vihar, Pune- 411048, Maharashtra</p>
      </div>
      <div className="flex items-center gap-4">
        <Phone className="h-6 w-6" style={{ color: "#fcd129" }} />
        <p>+91 8669888996</p>
      </div>
      <div className="flex items-center gap-4">
        <Mail className="h-6 w-6" style={{ color: "#fcd129" }} />
        <p>hello@driwe.in</p>
      </div>
      <div className="flex items-center gap-4">
        <Clock className="h-6 w-6" style={{ color: "#fcd129" }} />
        <div>
          <p>Mon–Fri 10AM–7PM IST</p>
          <p className="text-sm opacity-80">Executive meetings by appointment</p>
        </div>
      </div>
    </div>
  </>
...
export default function ContactPage() {
  ...
  const handleFormSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    setTimeout(() => {
      [nameRef, emailRef, phoneRef, inquiryRef, messageRef].forEach((ref) => {
        ...
        <div className="lg:w-1/2 lg:sticky lg:top-24 self-start">
          <FadeInOnScroll>
            <div className={`glass-card rounded-2xl p-8 shadow-2xl pulse-glow ${isShaking ? "animate-shake" : ""}`}>
              ...
              <form
                action="https://docs.google.com/forms/d/e/1FAIpQLSdIHfRQ9K61583HCDrEJasU7qY87sNoe3nDBBuaBcGxqGdYKA/formResponse"
                method="POST"
                ...
                className="space-y-6"
                 noValidate
              >
                <iframe name="hidden_iframe" className="hidden" title="hidden"></iframe>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium mb-2">Full Name *</label>
                    <input ref={nameRef} name="entry.2132482261" type="text" required className={`w-full rounded-lg px-4 py-3 focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition ${errors.name ? "border-red-500" : ""}`} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-2">Business Email *</label>
                    <input ref={emailRef} name="entry.1162438489" type="email" required className={`w-full rounded-lg px-4 py-3 focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition ${errors.email ? "border-red-500" : ""}`} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-2">Phone Number</label>
                    <input ref={phoneRef} name="entry.167981949" type="tel" className="w-full rounded-lg px-4 py-3 focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-2">Inquiry Type *</label>
                    <select ref={inquiryRef} name="entry.1116970278" required className="w-full rounded-lg px-4 py-3 focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition">
                ...
                <div>
                  ...
                  <textarea ref={messageRef} name="entry.171932238" rows={5} required placeholder="Tell us about your interest in Driwe..." className={`w-full rounded-lg px-4 py-3 focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition ${errors.message ? "border-red-500" : ""}`} />
                </div>

                <button type="submit" disabled={formStatus === "sending"} className="send-button bg-[#fcd129] font-bold px-8 py-3 rounded-lg flex items-center gap-2 min-w-[180px] justify-center hover:bg-[#eab308] transition">
                  {formStatus === "sending" ? (
                    <>
          ...
          <FadeInOnScroll delay={400}>
            <div className="glass-card rounded-2xl p-8 shadow-lg">
              ...
              <div className="flex flex-wrap gap-4">
                <a href="https://www.facebook.com/profile.php?id=61575003763571" target="_blank" rel="noopener noreferrer" className="bg-[#1877F2] p-3 rounded-full">
                  <FacebookIcon className="h-6 w-6" />
                </a>
                <a href="https://www.instagram.com/driwe.in" target="_blank" rel="noopener noreferrer" className="bg-[#E1306C] p-3 rounded-full">
                  <Instagram className="h-6 w-6" />
                </a>
                <a href="https://www.linkedin.com/company/driwe-app" target="_blank" rel="noopener noreferrer" className="bg-[#0A66C2] p-3 rounded-full">
                  <Linkedin className="h-6 w-6" />
                </a>
                <a href="https://www.threads.com/@driwe.in" target="_blank" rel="noopener noreferrer" className="bg-black p-3 rounded-full">
            ...
            <div className="glass-card rounded-2xl p-8 shadow-lg">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div className="text-center">
                  <TrendingUp className="h-10 w-10 mx-auto" style={{ color: "#fcd129" }} />
                  <p className="font-bold mt-2">Investor Relations</p>
                  <p>hello@driwe.in</p>
                </div>
                <div className="text-center">
                  <Handshake className="h-10 w-10 mx-auto" style={{ color: "#fcd129" }} />
                  <p className="font-bold mt-2">Partnerships</p>
                  <p>hello@driwe.in</p>
                </div>
                <div className="text-center">
                  <Newspaper className="h-10 w-10 mx-auto" style={{ color: "#fcd129" }} />
                  <p className="font-bold mt-2">Media & Press</p>
                  <p>hello@driwe.in</p>
                </div>
              </div>
            </div>
